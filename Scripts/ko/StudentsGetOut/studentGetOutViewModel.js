...
function StudentGetOutViewModel(modalConfirm) {
    var self = this;
    ...
    self.InitTableEstudent = function () {
        $('#tblEstudents').DataTable({
            ajax: {
                url: urlApi + "/RestController.php?view=out&idRuta="+self.idruta(),
                dataSrc: 'data'
            },
            order: [[0, "desc"]],
            columns: [
			...
			columnDefs: [
                {
                    ...
                    "mRender": function (data) {
                        ...
                        if(data=='1')
                        {
                            buttons = '<button id="btnSi" type="button" class="btnGrid btn-primary active" data-toggle="button" aria-pressed="true" autocomplete="off">'+
                                        'Si' +'</button>'+
                               ...
                               buttons= '<button id="btnSi" type="button" class="btnGrid btn-primary" data-toggle="button" aria-pressed="false" autocomplete="off">'+
                                        'Si' +'</button>'+
                                        '<button id="btnNo" type="button" class="btnGrid btn-primary active" data-toggle="button" aria-pressed="true" autocomplete="off">'+
                                        'No' +'</button>';
                        }else{
                            buttons= '<button id="btnSi" type="button" class="btnGrid btn-primary" data-toggle="button" aria-pressed="false" autocomplete="off">'+
                                        'Si' +'</button>'+
